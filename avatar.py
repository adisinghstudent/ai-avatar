...
def prompt_engineering(messages): 
    messages.append(
        {"role": "system", 
          "content": "Start each answer with hey bro, use cheesy pickup lines as answers, Add the winking emoji whenever possible, use maximum 40 words" }
        )
...
title_placeholder.title("Rizzle talks")


subtitle_placeholder = st.empty()
subtitle_placeholder.write("Hey Bro, I'm Rizztoffer here for whatever;)")
...
with rightColumn:
    with st.container(height=800, border=False):
        ...
        prompt = chat_input_placeholder.chat_input('What´s on your mind?')
        ...
        if prompt:
            ...
            with st.chat_message("assistant", avatar = 'images/avatar.png'):
                ...
                text = prompt
                text = gpt(text, messages)
                message.markdown(text)
                if not ANIMATE:
                    text2speech(text) # add text to speech here and remove 'pass'
